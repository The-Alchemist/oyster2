...
public class QueryBuilderAdapter {
	...
	public static Map<String,Object> getFilters(String queryTargetClass,
		...
		for (Entry<String,Object> entry : filterValues.entrySet()) {
			getFilter(queryTargetClass,entry,filters);
			/*
	...
	private static void getFilter(String queryTargetClass, Entry<String,Object> entry,Map<String,Object> builtFilters) {
		...
		if (value.getMulti()) {
			...
			if (isBranch) {
				...
				builtFilters.put(entry.getKey(),filterBranch);
			}
			else {
				//nested query
				//for now we have three cases:
				...
				// 3. -	create a name branch in the nested query
				nestedQueryFilters = makeNestedQuery(entry.getKey(),value);
				builtFilters.put(entry.getKey(),nestedQueryFilters);
			}
		}
		else {
			//simple filter
			try {
				simpleFilter = makeFilter(value);
				builtFilters.put(entry.getKey(),simpleFilter);
			} catch (ParseException e) {
				e.printStackTrace();
				throw new RuntimeException(e);
			}
