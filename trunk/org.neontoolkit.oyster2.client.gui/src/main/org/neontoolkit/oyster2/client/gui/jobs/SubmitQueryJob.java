...
public class SubmitQueryJob extends WorkspaceJob {
	...
	public IStatus runInWorkspace(IProgressMonitor monitor)
		...
		try {
			...
			*/
			...
			queryPortAdapter.setTargetService(targetService);
			queryPortAdapter.setTimeoutInMiliseconds(soTimeout);
	...
	private void openResultsView() {
		Display.getDefault().asyncExec(new Runnable() {
			public void run() {
				...
				try {
					String secId = getNextSecondaryId();
					
					SearchResultsView view = (SearchResultsView)
						page.showView(PerspectiveFactory.OYSTER2_RESULTS_VIEW_ID,secId,
	...
	private static synchronized String getNextSecondaryId() {
		resultsViewSecondaryId++;
		String id = String.valueOf(resultsViewSecondaryId);
		
		return PerspectiveFactory.OYSTER2_RESULTS_VIEW_SECONDARY_ID + id;
	}
