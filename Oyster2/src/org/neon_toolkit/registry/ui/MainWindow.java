...
import org.neon_toolkit.registry.oyster2.SearchingScope;
import org.neon_toolkit.registry.ui.action.AboutOyster2Action;
import org.neon_toolkit.registry.ui.action.EditEntryAction;
import org.neon_toolkit.registry.ui.action.ExitAction;
import org.neon_toolkit.registry.ui.action.ImportMappingOntology;
import org.neon_toolkit.registry.ui.action.ImportOntologyDocument;
import org.neon_toolkit.registry.ui.action.NewEntryAction;
import org.neon_toolkit.registry.ui.action.OpenPreferencesAction;
import org.neon_toolkit.registry.ui.action.RemoveAllEntriesAction;
import org.neon_toolkit.registry.ui.action.RemoveEntryAction;
import org.neon_toolkit.registry.ui.action.SaveEntryAction;
...
public class MainWindow extends ApplicationWindow {
	...
	private AboutOyster2Action aboutBibsterAction = new AboutOyster2Action(this);
...
//		--Button Panel--------------------------------------
		...
		detailViewer.addLinkSelectionListener(new LinkSelectionListener() {
			public void linkSelected(ViewerLink link) {
				...
				else if(link.getLinkType().equals(Constants.ontologyOMVLocation)
				){
					LinkedList<Condition> searchConditions = new LinkedList<Condition>();
					Condition condition=null;
					condition = new Condition(Constants.pomvCondition+Constants.ontologyOMVLocation, link.getLinkedValue(), false);
					searchConditions.addFirst(condition);
					performDataSearch(searchConditions);
				}
				else  {
	...
	private void performDataSearch() {
		...
		if (wildCard != null && !wildCard.equals("")) {
			keywordSearch=wildCard;
			//Condition condition = new Condition(null, wildCard, true );			
			//searchConditions.addFirst(condition);
		}
		else keywordSearch="";
	...
	public static String whichName(String which) {
		...
		if (which.equals(Constants.omvCondition+Constants.numberOfClasses)) {
			return "Classes";
		}
		if (which.equals(Constants.omvCondition+Constants.numberOfProperties)) {
			return "Properties";
		}
		if (which.equals(Constants.omvCondition+Constants.numberOfIndividuals)) {
			return "Individuals";
		}
		if (which.equals(Constants.omvCondition+Constants.numberOfAxioms)) {
			return "Axioms";
		}
