	...
	private static ObjectSomeValuesFrom createObjectSomeValuesFrom(String uri, String propertyValue, Ontology ontologySearch) {
		...
		try{
			...
			if (uri.equalsIgnoreCase(Constants.OWLODMURI+Constants.objectProperty)) {
				Individual oIndividual =KAON2Manager.factory().individual(propertyValue);
				org.neon_toolkit.owlodm.api.OWLEntity.ObjectProperty replyTemp=(org.neon_toolkit.owlodm.api.OWLEntity.ObjectProperty)ProcessOWLEntityIndividuals.processOWLEntityIndividual(oIndividual, oIndividual.getDescriptionsMemberOf(ontologySearch).iterator().next().toString(), ontologySearch); //oReferences
	...
	private static ObjectOneOf createObjectOneOf(String uri, String propertyValue, Ontology ontologySearch) {
		...
		try{
			if (uri.equalsIgnoreCase(Constants.OWLODMURI+Constants.individuals)) {
				Individual oIndividual =KAON2Manager.factory().individual(propertyValue);
				org.neon_toolkit.owlodm.api.OWLEntity.Individual replyTemp=(org.neon_toolkit.owlodm.api.OWLEntity.Individual)ProcessOWLEntityIndividuals.processOWLEntityIndividual(oIndividual, oIndividual.getDescriptionsMemberOf(ontologySearch).iterator().next().toString(), ontologySearch); //oReferences
	...
	private static DataSomeValuesFrom createDataSomeValuesFrom(String uri, String propertyValue, Ontology ontologySearch) {
		...
		try{
			if (uri.equalsIgnoreCase(Constants.OWLODMURI+Constants.dataProperties)) {
				Individual oIndividual =KAON2Manager.factory().individual(propertyValue);
				org.neon_toolkit.owlodm.api.OWLEntity.DataProperty replyTemp=(org.neon_toolkit.owlodm.api.OWLEntity.DataProperty)ProcessOWLEntityIndividuals.processOWLEntityIndividual(oIndividual, oIndividual.getDescriptionsMemberOf(ontologySearch).iterator().next().toString(), ontologySearch); //oReferences
